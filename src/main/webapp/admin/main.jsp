...
<title>知识库系统后台管理页面-Powered by CF</title>
...
<script type="text/javascript">
	...
	function openTab(text, url, icon) {
		...
		if ($("#tabs").tabs("exists", text)) {
			...
			//如果不存在，则新建一个
			var content="<iframe frameborder=0 scrolling='auto' style='width:100%;height:100%' src='${kbms}/admin/"+url+"'></iframe>";
			$("#tabs").tabs("add",{
				title:text,
				closable:true,
				iconCls:icon,
	...
	function openPasswordModifyDialog(){
		...
		url="${kbms}/admin/user/modifyPassword.do?id=${currentUser.id}";
	}
	...
	function logout(){
		$.messager.confirm("系统提示","您确定要退出系统吗？",function(r){
			if(r){
				window.location.href='${kbms}/admin/user/logout.do';
			} 
		 });
	}
	...
	function refreshSystem() {
		$.post("${kbms}/admin/system/refreshSystem.do",{},function(result){
			if(result.success){
...
<body class="easyui-layout">
	<div region="north" style="height:78px;background-color:#E0ECFF;">
		<table style="padding: 5px" width="100%">
			<tr>
				<td width="50%">
					<img alt="logo" src="${kbms}/static/images/logo.png">
				</td>
	...
	<div region="west" style="width:200px" title="导航菜单" split="true">
		<div class="easyui-accordion" data-options="fit:true,border:false">
			<div title="常用操作" data-options="selected:true,iconCls:'icon-item'" style="padding: 10px">
				<a href="javascript:openTab('写文章','writeArticle.jsp','icon-writeblog')" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-writeblog'" style="width: 150px">写文章</a>
			...
			<c:if test="${sessionScope.currentUser.manage == 1 }">
