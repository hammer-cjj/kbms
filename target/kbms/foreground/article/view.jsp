...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(content==null || content==''){
			...
			$.post("${pageContext.request.contextPath}/comment/save.do",{'content':content,'imageCode':imageCode,'article.id':'${article.id}'},function(result){
				if(result.success){
...
<div class="data_list">
	...
	<div>
	   <div class="blog_title"><h3><strong>${article.title }</strong></h3></div>
		<div class="blog_info">
			发布时间：『 <fmt:formatDate value="${article.releaseDate }" type="date" pattern="yyyy-MM-dd HH:mm"/>』&nbsp;&nbsp;文章类别：${article.articleType.typeName}&nbsp;&nbsp;阅读(${article.clickHit}) 评论(${article.replyHit})
		</div>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="keyWord" items="${keyWords }">
						&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/article/q.html?q=${keyWord}" target="_blank">${keyWord }</a>&nbsp;&nbsp;
					</c:forEach>
				</c:otherwise>
			</c:choose>
		</div>
